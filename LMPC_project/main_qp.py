...
Q = np.kron(np.eye(N), np.diag([100, 100, 1, 1]))
R = np.diag([0.1,0.1])
...
for t in range(steps):
    # QP構築
    P, q, G, h = build_mpc_qp(A, B, N, Q, R, x_ref, x, umin, umax)

    # 解く
    u_seq = solve_qp(P, q, G, h, solver="osqp")
    if u_seq is None:
        print(f"QP solve failed at step {t}")
        break

    u = u_seq[0:nu] # Extract the first control input from the sequence
    x = A @ x + B @ u
    X_log.append(x)
    U_log.append(u)
    T.append(T[-1] + dt)
